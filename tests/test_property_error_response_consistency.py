...
async def test_not_found_error_format_update(non_existent_id):
    ...
    async with AsyncSessionLocal() as session:
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            response = await client.put(f"/api/resources/{non_existent_id}", json={
                "name": "Updated Name"
            })
...
async def test_circular_dependency_error_format():
    ...
    async with AsyncSessionLocal() as session:
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            # Create resource A
            response_a = await client.post("/api/resources", json={
                "name": "Resource A",
                "dependencies": []
            })
            ...
            response_b = await client.post("/api/resources", json={
                "name": "Resource B",
            ...
            response = await client.put(f"/api/resources/{resource_a_id}", json={
            ...
            assert "circular" in response_json["message"].lower() or \
                   "circular" in response_json["error"].lower(), \
...
async def test_invalid_dependency_error_format(invalid_dep_id):
    ...
    async with AsyncSessionLocal() as session:
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            response = await client.post("/api/resources", json={
                "name": "Resource with invalid dep",
                "dependencies": [invalid_dep_id]
            })
