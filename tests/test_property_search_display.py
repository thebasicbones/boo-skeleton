...
async def test_property_search_displays_topological_order(
    ...
    for name, description, dep_indices in dag_structure:
        ...
        assert response.status_code == 201, \
            f"Failed to create resource: {response.text}"
        ...
        for resource in search_results:
            ...
            for dep_id in dependencies:
                if dep_id in result_ids:
                    # Find positions
                    dep_position = next(i for i, r in enumerate(search_results) if r['id'] == dep_id)
                    resource_position = next(i for i, r in enumerate(search_results) if r['id'] == resource_id)
                    
                    assert dep_position < resource_position, \
...
async def test_property_search_empty_query_returns_all_in_topological_order(
    ...
    assert positions['A'] < positions['B'], \
        "Resource A should appear before B (B depends on A)"
    assert positions['B'] < positions['C'], \
        "Resource B should appear before C (C depends on B)"
...
async def test_property_search_with_query_maintains_topological_order(
    ...
    assert pos_a < pos_b, \
        "Resource A should appear before B in search results (B depends on A)"


@pytest.mark.asyncio
@pytest.mark.property
async def test_property_search_complex_dag_topological_order(
    search_test_client: AsyncClient
):
    """
    Feature: fastapi-crud-backend, Property 22: Search displays topological order
    ...
    response_a = await search_test_client.post(
        "/api/resources",
        json={"name": "A", "description": "Node A", "dependencies": []}
    )
    assert response_a.status_code == 201
    resource_a = response_a.json()
    ...
    response_b = await search_test_client.post(
        "/api/resources",
        json={"name": "B", "description": "Node B", "dependencies": []}
    )
    assert response_b.status_code == 201
    resource_b = response_b.json()
    ...
    assert all(node in positions for node in ['A', 'B', 'C', 'D', 'E']), \
...
@pytest.mark.asyncio
@pytest.mark.property
async def test_property_search_no_dependencies_consistent_order(
    search_test_client: AsyncClient
):
    """
    Feature: fastapi-crud-backend, Property 22: Search displays topological order
    ...
    assert ids_1 == ids_2, \
        "Resources with no dependencies should appear in consistent order"
