...
async def test_mongodb_schema_field_completeness(resource_data):
    ...
    try:
        ...
        for field in required_fields:
            assert field in created_resource, \
        ...
        assert created_resource['id'] == raw_document['_id'], \
            "Repository should map '_id' to 'id'"
        
        assert created_resource['name'] == raw_document['name'], \
            "Name field should be consistent between repository and MongoDB"
        
        assert created_resource['description'] == raw_document['description'], \
            "Description field should be consistent between repository and MongoDB"
        
        assert created_resource['dependencies'] == raw_document['dependencies'], \
...
async def test_mongodb_schema_field_completeness_after_update(resource_data):
    ...
    try:
        ...
        for field in required_fields:
            assert field in updated_resource, \
        ...
        assert updated_resource['id'] == created_resource['id'], \
            "ID should not change after update"
        
        assert updated_resource['dependencies'] == created_resource['dependencies'], \
        ...
        assert round_to_milliseconds(updated_resource['created_at']) == round_to_milliseconds(created_resource['created_at']), \
        ...
        assert updated_resource['updated_at'] >= created_resource['updated_at'], \
...
async def test_mongodb_schema_field_completeness_after_retrieval(resource_data):
    ...
    try:
        ...
        for field in required_fields:
            assert field in retrieved_by_id, \
        ...
        assert retrieved_by_id['description'] is None or isinstance(retrieved_by_id['description'], str)
        ...
        our_search_result = next((r for r in search_results if r['id'] == created_resource['id']), None)
        
        if our_search_result:  # Only check if found (search might not always return it)
            for field in required_fields:
                assert field in our_search_result, \
            ...
            assert our_search_result['description'] is None or isinstance(our_search_result['description'], str)
