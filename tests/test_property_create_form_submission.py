...
async def test_property_create_form_submission_without_dependencies(
    ...
    create_response = await form_test_client.post(
        "/api/resources",
        json=form_payload
    )
    ...
    if resource_data.description is None:
        assert created_resource["description"] is None or created_resource["description"] == "", \
    ...
    assert created_resource["dependencies"] == resource_data.dependencies, \
    ...
    assert found_resource is not None, \
        "Newly created resource not found in resource list"
    
    # Verify the resource in the list matches what was created
    assert found_resource["name"] == resource_data.name, \
    ...
    if resource_data.description is None:
        ...
        assert found_resource["description"] == resource_data.description, \
            "Resource description in list doesn't match form data"
    
    assert found_resource["dependencies"] == resource_data.dependencies, \
...
async def test_property_create_form_submission_with_dependencies(
    ...
    assert "dependencies" in created_resource, \
        "Created resource missing dependencies field"
    
    assert len(created_resource["dependencies"]) == len(selected_dependencies), \
    ...
    for dep_id in selected_dependencies:
        assert dep_id in created_resource["dependencies"], \
    ...
    assert len(found_resource["dependencies"]) == len(selected_dependencies), \
        "Dependencies not preserved in resource list"
    
    for dep_id in selected_dependencies:
        assert dep_id in found_resource["dependencies"], \
...
async def test_property_create_form_clears_after_submission(form_test_client: AsyncClient):
    ...
    first_response = await form_test_client.post(
        "/api/resources",
        json=first_payload
    )
    ...
    second_response = await form_test_client.post(
        "/api/resources",
        json=second_payload
    )
...
async def test_property_create_form_validation_errors_displayed(form_test_client: AsyncClient):
    ...
    response = await form_test_client.post(
        "/api/resources",
