...
async def test_property_edit_form_population(
    ...
    assert get_response.status_code == 200, \
        f"Expected 200 OK, got {get_response.status_code}"
    ...
    assert retrieved_resource["name"] == initial_data.name, \
        "Resource name doesn't match original data"
    
    # Handle None description
    if initial_data.description is None:
        assert retrieved_resource["description"] is None or retrieved_resource["description"] == "", \
            "Resource description should be None or empty"
    else:
        assert retrieved_resource["description"] == initial_data.description, \
            "Resource description doesn't match original data"
    
    assert retrieved_resource["dependencies"] == initial_data.dependencies, \
...
async def test_property_update_form_submission(
    ...
    assert updated_resource["id"] == resource_id, \
        "Resource ID changed after update"
    
    assert updated_resource["name"] == normalized_name, \
    ...
    if expected_description is None or expected_description == "":
        assert updated_resource["description"] is None or updated_resource["description"] == "", \
    ...
    assert persisted_resource["name"] == normalized_name, \
        "Updated name not persisted"
    
    if expected_description is None or expected_description == "":
        assert persisted_resource["description"] is None or persisted_resource["description"] == "", \
            "Persisted description should be None or empty"
    else:
        assert persisted_resource["description"] == expected_description, \
    ...
    assert found_resource is not None, \
        "Updated resource not found in resource list"
    
    assert found_resource["name"] == normalized_name, \
        "Updated resource name in list doesn't match form data"
    
    if expected_description is None or expected_description == "":
        assert found_resource["description"] is None or found_resource["description"] == "", \
            "Resource description in list should be None or empty"
    else:
        assert found_resource["description"] == expected_description, \
...
async def test_property_dependency_modification_in_update(
    ...
    assert "dependencies" in updated_resource, \
        "Updated resource missing dependencies field"
    
    assert len(updated_resource["dependencies"]) == 4, \
        f"Expected 4 dependencies after update, got {len(updated_resource['dependencies'])}"
    
    assert set(updated_resource["dependencies"]) == set(updated_dependencies), \
    ...
    assert len(persisted_resource["dependencies"]) == 4, \
        "Updated dependencies not persisted"
    
    assert set(persisted_resource["dependencies"]) == set(updated_dependencies), \
    ...
    assert len(reduced_resource["dependencies"]) == 2, \
        f"Expected 2 dependencies after reduction, got {len(reduced_resource['dependencies'])}"
    
    assert set(reduced_resource["dependencies"]) == set(reduced_dependencies), \
    ...
    assert len(empty_resource["dependencies"]) == 0, \
    ...
    assert found_resource is not None
    assert len(found_resource["dependencies"]) == 0, \
...
async def test_property_update_circular_dependency_prevention(update_test_client: AsyncClient):
    ...
    assert len(resource_a_data["dependencies"]) == 0, \
