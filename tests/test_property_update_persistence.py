...
async def test_mongodb_update_persistence(initial_data, update_data):
    ...
    try:
        ...
        assert round_to_milliseconds(retrieved_resource['created_at']) == round_to_milliseconds(original_created_at)
...
async def test_backend_equivalence_update_persistence(initial_data, update_data):
    ...
    try:
        ...
        async with async_session() as session:
            ...
            mongodb_updated = resource_to_dict(mongodb_updated_obj)
            ...
            def round_to_milliseconds(dt):
                return dt.replace(microsecond=(dt.microsecond // 1000) * 1000)
            assert round_to_milliseconds(mongodb_retrieved['created_at']) == round_to_milliseconds(mongodb_created['created_at'])
