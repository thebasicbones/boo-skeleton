...
from app.models.sqlalchemy_resource import Resource, Base
...
async def export_sqlite_to_json(sqlite_url: str, output_file: str, stats: MigrationStats) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if existing_count > 0:
            ...
            if response.lower() == 'yes':
                ...
                print(f"      Keeping existing data, will attempt to insert new documents")
        ...
        if resources_data:
            ...
            for i in range(0, len(resources_data), batch_size):
                ...
                try:
                    ...
                    print(f"      Imported {stats.resources_imported}/{len(resources_data)} resources...", end='\r')
                except Exception as e:
                    ...
                    if 'duplicate key' in str(e).lower():
                        ...
                        print(f"      Warning: Some documents already exist, skipped duplicates")
                    else:
                        raise
    ...
    try:
        ...
        for resource_id, sqlite_resource in sqlite_resources.items():
            ...
            if sqlite_deps != mongo_deps:
                error_msg = f"Dependencies mismatch for {resource_id}: {sqlite_deps} != {mongo_deps}"
                stats.errors.append(error_msg)
                validation_passed = False
...
async def migrate(
    ...
    try:
        ...
        validation_passed = await validate_migration(sqlite_url, mongodb_url, mongodb_db, stats, dry_run)
