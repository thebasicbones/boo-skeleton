...
def get_field(obj: dict | Any, field: str) -> Any:
    """
...
def get_dependencies(obj: dict | Any) -> list[str]:
    """
...
async def test_create_resource(db_backend):
    ...
    data = ResourceCreate(name="Test Resource", description="A test resource", dependencies=[])
...
async def test_create_with_dependencies(db_backend):
    ...
    data = ResourceCreate(
        name="Main Resource", description="Has dependencies", dependencies=[dep1_id, dep2_id]
    )
    resource = await repository.create(data)
...
async def test_search_by_description(db_backend):
    ...
    await repository.create(
        ResourceCreate(name="Service A", description="Python backend", dependencies=[])
    )
    await repository.create(
        ResourceCreate(name="Service B", description="React frontend", dependencies=[])
    )
...
async def test_delete_cascade(db_backend):
    ...
    resource_b = await repository.create(ResourceCreate(name="B", dependencies=[resource_a_id]))
    resource_b_id = get_field(resource_b, "id")

    resource_c = await repository.create(ResourceCreate(name="C", dependencies=[resource_b_id]))
