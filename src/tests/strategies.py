...
@st.composite
def resource_update_strategy(
    draw, include_name=True, include_description=True, include_dependencies=False
):
    """
    ...
    if include_dependencies:
        dependencies = draw(st.one_of(st.none(), dependency_list_strategy(min_size=0, max_size=5)))
...
def resource_create_with_existing_dependencies_strategy(draw, existing_ids):
    ...
    if existing_ids:
        ...
        if num_deps > 0:
            dependencies = draw(
                st.lists(
                    st.sampled_from(existing_ids), min_size=num_deps, max_size=num_deps, unique=True
                )
            )
        else:
            dependencies = []
...
name_only_update_strategy = resource_update_strategy(
    include_name=True, include_description=False, include_dependencies=False
)

# Strategy for partial updates (only description)
description_only_update_strategy = resource_update_strategy(
    include_name=False, include_description=True, include_dependencies=False
)

# Strategy for full updates (all fields)
full_update_strategy = resource_update_strategy(
    include_name=True, include_description=True, include_dependencies=True
)
