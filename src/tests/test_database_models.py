...
async def test_resource_with_dependencies(repository):
    ...
    resource_a = await repository.create(
        ResourceCreate(name="Resource A", description="Base resource A", dependencies=[])
    )
    resource_b = await repository.create(
        ResourceCreate(name="Resource B", description="Base resource B", dependencies=[])
    )

    # Create resource that depends on A and B
    resource_c = await repository.create(
        ResourceCreate(
            name="Resource C",
...
async def test_cascade_delete(repository):
    ...
    resource_a = await repository.create(ResourceCreate(name="Resource A", dependencies=[]))
    resource_b = await repository.create(
        ResourceCreate(name="Resource B", dependencies=[resource_a.id])
    )
