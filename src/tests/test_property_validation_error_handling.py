...
def resource_create_strategy(draw):
    ...
    dependencies = draw(st.lists(st.text(min_size=1, max_size=36), max_size=5, unique=True))
...
def resource_update_strategy(draw):
    ...
    # Generate unique dependencies
    dependencies = draw(
        st.one_of(st.none(), st.lists(st.text(min_size=1, max_size=36), max_size=5, unique=True))
    )
...
def validation_error_strategy(draw):
    ...
    error_doc = {"index": 0, "code": error_code, "errmsg": error_message}
...
@given(resource_data=resource_create_strategy(), validation_error=validation_error_strategy())
...
async def test_update_operation_validation_error_handling(
    resource_id, update_data, validation_error
):
    """
    Feature: mongodb-integration, Property 11: Validation error handling
    Validates: Requirement 6.2
...
async def test_validation_error_preserves_details(validation_error):
    ...
    resource_data = ResourceCreate(
        name="Test Resource", description="Test Description", dependencies=[]
    )
...
@pytest.mark.asyncio
@settings(max_examples=100, suppress_health_check=[HealthCheck.function_scoped_fixture])
@given(resource_data=resource_create_strategy(), error_message=st.text(min_size=10, max_size=200))
async def test_validation_error_message_clarity(resource_data, error_message):
    ...
    error_doc = {"index": 0, "code": 121, "errmsg": f"Document failed validation: {error_message}"}
    validation_error = WriteError(f"Document failed validation: {error_message}", 121, error_doc)
